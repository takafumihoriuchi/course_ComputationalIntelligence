...
と期待されたが、現実はそう甘くなかった。誤差逆伝播法を用いてもなお、ディープなニューラルネットワークで高い性能を出すためには、次のようなさらなる課題が存在した。

- **勾配喪失と勾配爆発**  
誤差逆伝播では誤差を重みで吸収するが、その誤差の勾配が伝搬に従い数層（３、４程度）で消える。逆にある閾値を超えると勾配が拡大し、勾配が爆発的に大きくなる。これはシグモイド関数の微分が、xが大きくなると０に近づくことによる。（以上 資料よりそのまま転用）

- **過学習**  
学習が過度になり、判定が厳格になってしまう問題。汎用性が失われてしまい、本来の目的である未知のデータの分類での性能が低下してしまう。

- **シグモイド関数の課題**  
シグモイド関数では、微分をするとどんどん小さくなっていってしまう。そこで、ReLU関数を用いることで、勾配喪失を防げる。ReLU関数は、x=0では連続微分可能ではないが、x>0では微分すると1になるため、勾配喪失が発生しない。ちなみに「ReLU」とは「Rectified Linear Unit」の略であり、またの名を「Ramp関数」という。

- **重みの初期化の設定方法**  
"try and error"で重みの初期値を設定することが多いが、この重みの設定が学習器の性能に大きな影響を及ぼす。試行錯誤ではない、より良い手法が欲しいということ。  
  
#### オートエンコーダ

勾配喪失と重みの初期値の問題を解決する手法として、オートエンコーダという手法がある。オートエンコーダは別名を自己符号化機といい、学習器のトレーニングを始める前にその結合荷重の初期値を決定するために行う処理である。詳細は割愛するが、大枠は、入力側から、隣接する二つの層を取り出し、その鏡面の構造を作成し、入力と出力が一致するように誤差逆伝播法で重みを更新していく、というものである。  


## おまけの話題

ニューラルネットワークのモデルには、例えば次のようなものがある。
...
Q関数をニューラルネットワークで実現する。状態のパラメータ値を入力し、適切な行為を出力する手法がある。また、状態と行為を入力し、その価値を出力する手法もある。最も一般的なのは、**その場面（状況）の画像を入力として与え、CNNを通して状況を解析し、適切な行為と価値を出力する**もの。  
